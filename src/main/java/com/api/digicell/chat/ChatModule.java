...
public class ChatModule {
        ...
        server.addEventListener(SocketConfig.EVENT_PING, UserPingRequest.class, (socketClient, pingRequest, ackSender) -> {
            ...
            try {
                ...
                if (user == null) {
                    ...
                    log.info("🔄 RECOVERY: Created fallback user object - UserId: {}, Email: {}, UserName: {}, Last ping: {}", 
                            user.getUserId(), user.getEmail(), user.getUserName(), formatPingTime(user.getLastPingTime()));
                } else {
                    log.debug("🔄 Existing user found, updating ping time to: {}", formatPingTime(user.getLastPingTime()));
                ...
                log.debug("✅ Ping processed for user: {}, email: {}, userName: {}, last ping: {} and added to tenant pools", 
                        user.getUserId(), user.getEmail(), user.getUserName(), formatPingTime(user.getLastPingTime()));
    ...
    /**
     * Helper method to format milliseconds timestamp to readable date format with centiseconds
     */
    private String formatPingTime(long pingTimeMillis) {
        try {
            return Instant.ofEpochMilli(pingTimeMillis)
                    .atZone(ZoneOffset.UTC)
                    .format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss.SS"));
        } catch (Exception e) {
            return "Invalid timestamp: " + pingTimeMillis;
        }
    }
